const KEY = "AIzaSyAt-j4lH6mm3FzAAZ8K_NSjxgkWvQav_pc";
...
let testdata = [
  ...
  {
    title:
      "McLaren 720S Spider 2020 review - see why it&#39;s the ULTIMATE convertible supercar!",
  ...
  {
    title:
      "McLaren 720s with Novitec Kit in Kerala | ആരും ഒന്ന് നോക്കി പോകും! | Flywheel Malayalam",
...
$(document).ready(() => {
      ...
      let videoDiv = `
            <div class="video-list-item shadow-sm d-flex" >
                ...
                <div class="list-item-meta text-muted d-flex flex-column justify-content-between">
                    <div>    
                        <h6 class="m-0 p-0 fs-5 fw-bold text-muted">${video.title.slice(
                          0,
                          50
                        )}</h6>
                        <p class="mt-2 m-0 p-0 truncate-desc text-muted">${
                          video.desc
                        }</p>
                    </div>
                    <div>
                        <button class="m-0 p-0 fw-bold btn btn-danger btn-sm channel-btn p-2 font2">${
                          video.channel.split(" ")[0]
                        }</button>
                        <button class="m-0 p-0 fw-bold btn btn-outline-danger btn-sm play-btn ms-1 p-2 font2" id=${
                          video.id
                        }><i class="fa fa-play play-btn me-1 " aria-hidden="true"></i>Play</button>
                    </div>
                </div>
            </div>`;
  ...
  function playNewVideo(id, title, desc) {
    ...
    $(".video-title").append(
      `<h1 class="video-title-text fw-bold border-bottom border-dark text-danger fs-1">${title}</h1>`
    );
    $(".video-desc").append(
      `<p class="video-desc-text text-muted">${desc}</p>`
    );
  }
